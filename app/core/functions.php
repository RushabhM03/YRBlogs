...
function old_checked($key, $value, $default=''){
...
function old_select($key, $default=''){
    if(!empty($_POST[$key])){
        return " checked ";
    }
    return $default;
...
function get_pagination_vars(){
    // set pagination
    $page_no = $_GET['page'] ?? 1;
    $page_no = empty($page_no) ? 1 : (int)$page_no;
    ...
    if(!strstr($query_string, "page=")){
        $query_string .= '&page='.$page_no; 
    }
    $query_string = trim($query_string, '&');
    $current_link = $current_link. '?'. $query_string;

    $current_link = preg_replace("/page=.*/", "page=".$page_no, $current_link);
    $next_link = preg_replace("/page=.*/", "page=".($page_no+1), $current_link);
    $first_link = preg_replace("/page=.*/", "page=1", $current_link);
    $prev_page_no = $page_no < 2 ? 1 : $page_no - 1;
    $prev_link = preg_replace("/page=.*/", "page=".($prev_page_no), $current_link);
